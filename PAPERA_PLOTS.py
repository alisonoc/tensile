...
def second_derivative(material=None,
    ...
    for m in material:
        # ##USING PATH DIC FIND GET RELEVANT DATA
        df = pd.read_csv(os.path.join(res_pth['%s' % (m)], 'TRUE_INTERP.csv'), )
        # ##CONVERT STRAIN TO %
        df['TRUE_STRAIN_PC'] = df['TRUE_STRAIN'] * 100
        # ##READ IN WINDOW LENGTH INFORMATION
        wl = pd.read_csv(os.path.join(res_pth['%s' % (m)], 'WINDOW_LENGTH.csv'), index_col=0).reset_index(drop=True)
        ...
        # ##READ IN LINEAR DATA
        linear = pd.read_csv(
            os.path.join(res_pth['%s' % (m)], 'LINEAR_REGION_WL%s.csv' % (str(int(wl['WINDOW_LENGTH'])))))
        # ##GET MAX FORCE AND MAX DISPLACEMENT
        mf = df['TRUE_STRESS'].max()
        ...
        ax[0].scatter(y_df['TRUE_STRAIN_PC'],
                      ...
                      marker=marker_dic[m]['marker'],
                      color='b',
                      label='Dataset %s, $\sigma_y$: %s MPa' % (
                          marker_dic[m]['number'], round(y_df['TRUE_STRESS'].values[0], 1)))
...
res_dic={}
for m in materials:
    res_dic[m] = os.path.join(os.path.join(os.getcwd(), 'OUTPUT'), m)
    res_dic['PAPERA_PLOTS'] = os.path.join(os.path.join(os.getcwd(), 'OUTPUT'), 'PAPERA_PLOTS')
    res_dic[('save', m)] = os.path.join(os.path.join(os.path.join(os.getcwd(), 'OUTPUT'), 'PAPERA_PLOTS'),m)
...
experimental_results(material=materials,
					 res_pth=pdic,
					 marker_dic=marker_dic)
# ##PLOT ALL TRUE STRESS-STRAIN
true_stress_strain(material=materials,
				   res_pth=pdic,
				   marker_dic=marker_dic)
second_derivative(material=materials,
				  res_pth=res_dic,
				  marker_dic=marker_dic)
